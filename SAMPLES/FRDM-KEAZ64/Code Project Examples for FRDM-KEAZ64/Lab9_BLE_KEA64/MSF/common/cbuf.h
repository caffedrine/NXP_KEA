...
/** added by jd:
 * Advance the push index by N (see CBUF_AdvancePushIdx)
 */
#define	CBUF_AdvancePushIdxN(cbuf, cnt)		cbuf.m_putIdx += cnt;

/** added by jd:
 * Advance the pop index by N (see CBUF_AdvancePopIdx)
 */
#define CBUF_AdvancePopIdxN( cbuf, cnt )  cbuf.m_getIdx += cnt;

/** added by jd:
 * Set the push index to 0 meaning the pushing starts from beginning again.
 * The CBUF_Push does not reset the push index; the code will actually store the
 * items from 0 again if the Q is full, but the CBUF_Len() will report big number!
 *  */
#define	CBUF_ResetPushIdx(cbuf)		cbuf.m_putIdx = 0;
