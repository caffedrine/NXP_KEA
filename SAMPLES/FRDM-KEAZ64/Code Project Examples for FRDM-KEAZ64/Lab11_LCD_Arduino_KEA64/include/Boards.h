 ...
 *============================================================================*/
  ...
  // see MSF arduino.c for more details 
  #define digitalPinHasPWM(p)     ( (p) == 3 || ((p) == 5)  || ((p) >= 10 && (p) <= 13) )
#elif defined(__FRDM_KL25Z__)
  // see MSF arduino.c for more details 
  #define digitalPinHasPWM(p)     (((p) >= 0 && (p) <= 21) && !((p) >= 14 && (p) <= 15))
#else
...
// FRDM-KL25Z
#elif defined(__FRDM_KL25Z__)
#define TOTAL_ANALOG_PINS       MSF_ARDUINO_APINS    // 6
#define TOTAL_PINS              MSF_ARDUINO_PINS  // 22
#define VERSION_BLINK_PIN       13
//#define IS_PIN_DIGITAL(p)       ((p) >= 2 && (p) < (MSF_ARDUINO_PINS-MSF_ARDUINO_APINS)) // pin 0/1 used by Serial driver

#define IS_PIN_DIGITAL(p)       ((p) >= 2 && (p) < (MSF_ARDUINO_PINS)) // pin 0/1 used by Serial driver

#define IS_PIN_ANALOG(p)        ((p) >= 16 && (p) < TOTAL_PINS)
//#define IS_PIN_PWM(p)           digitalPinHasPWM(p)
#define IS_PIN_PWM(p)           ((p) >= 2 && (p)<= 13)
#define IS_PIN_SERVO(p)         0//((p) >= 0 && (p) - 2 < MAX_SERVOS) //??
// n/a #define IS_PIN_I2C(p)           ((p) == 20 || (p) == 21)
#define PIN_TO_DIGITAL(p)       (p)
//#define PIN_TO_ANALOG(p)        ((p) - MSF_ARDUINO_PINS)
#define PIN_TO_ANALOG(p)        ((p) - MFS_ARDUINO_PINS + MFS_ARDUINO_APINS)

#define PIN_TO_PWM(p)           PIN_TO_DIGITAL(p)
#define PIN_TO_SERVO(p)         ((p) - 2)

/ FRDM-KL25Z
#elif defined(__FRDM_KEA__)
#define TOTAL_ANALOG_PINS       MSF_ARDUINO_APINS    // 6
#define TOTAL_PINS              MSF_ARDUINO_PINS     // 22
#define VERSION_BLINK_PIN       13

#define IS_PIN_DIGITAL(p)       ((p) >= 0 && (p) < (MSF_ARDUINO_PINS)) // excluding analog pins
