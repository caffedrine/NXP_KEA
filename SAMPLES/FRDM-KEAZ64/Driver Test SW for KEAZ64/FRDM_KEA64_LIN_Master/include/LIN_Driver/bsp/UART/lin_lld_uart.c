...
/*--------------------------------------------------------------------*/
...
#else
  /* Gate the appropriate clock of your device for LIN communication. */
#endif

#if defined(MCU_SKEAZ1284)
  SIM_PINSEL1 |= SIM_PINSEL1_UART1PS_MASK;
#elif defined(MCU_SKEAZN642)
  /* There is no pin select for UART1 on KEAZN64. */
#else
  /* Write the appropriate pin select instruction, if any, for your device. */
#endif
  ...
  /* Set Callback. */
#if _UART1_ == 1
  UART_SetCallback(UART1, UART1_IRQHandler_LIN);
#elif _UART0_ == 1
  UART_SetCallback(UART0, UART0_IRQHandler_LIN);
#else
  /* Set the IRQ callback for the UART of your device. */
#endif
  /* Enter IDLE state */
  lin_goto_idle_state();
  ...
  )
{
  ...
  SIM_SCGC |= SIM_SCGC_UART1_MASK;
#else
  /* Gate the clock to the UART of your device. */
#endif

#if defined(MCU_SKEAZ1284)
  SIM_PINSEL1 |= SIM_PINSEL1_UART1PS_MASK; /* Set Pin out for UART1 at  PTF2 and PTF3 */
#elif defined(MCU_SKEAZN642)
  /* There are no alt pins for UART1 on KEAZN64. */
#else
  /* Write the SIM pin select for your device. */
#endif
...
#if _UART1_==1
  UART_SetCallback((UART_MemMapPtr)_pUART, (UART_CallbackType)UART1_IRQHandler_LIN);
#elif _UART0_==1
  UART_SetCallback((UART_MemMapPtr)_pUART, (UART_CallbackType)UART0_IRQHandler_LIN);
#else
  /* Set the correct UART for your device. */
#endif
