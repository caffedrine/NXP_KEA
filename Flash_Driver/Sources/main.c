...
//	printf("Erasing sector starting at 0x%.8X before write...", start_write_address);
...
//	printf("Writing '%s' (%d bytes) at address 0x%.8X...", payload, size, start_write_address);
	uint16_t flash_write_result;
	flash_write_result = FLASH_Program(start_write_address, payload, size);
